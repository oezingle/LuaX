...
local key_first=require"7b791f5a-9ab9-4f26-8ff8-7af2bb1b7b9b.util.key.key_first"
local function count_children_by_key(children_by_key,key) local count=0
local first,restkey=key_first(key)
for index,child in ipairs_with_nil(children_by_key,first) do if child then if child.class then count=count + 1 else local pass_key=index == first
