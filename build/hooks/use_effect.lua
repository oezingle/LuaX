...
if  not deps or  not table_equals(deps,old_deps,false) then if on_remove then on_remove() end
local callback_result=callback()
hookstate:set_value_silent(index,{["deps"] = deps,["on_remove"] = callback_result}) end
