...
local function count_children_by_key(children_by_key,key,include_virtual) local count=0
local first,restkey=key_first(key)
for index,child in ipairs_with_nil(children_by_key,first) do if child then if child.class then if child.class ~= VirtualElement and  not include_virtual then count=count + 1 end else local pass_key=index == first
