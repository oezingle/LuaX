...
function NativeElement:count_children_by_key(key) return count_children_by_key(self._children_by_key,key) end
...
set_child_by_key(self._children_by_key,key,child) end
...
local NativeTextElement=self._dependencies.NativeTextElement
local is_text=NativeTextElement and NativeTextElement:classOf(child.class) or false
if child.class ~= VirtualElement then self:delete_child(child_index,is_text) end end
