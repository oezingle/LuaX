...
describe("Inline", function()
    ...
    describe("handles errors", function()
        it("in code that cannot be transpiled", function()
            local ok, err = pcall(function()
                return Inline:transpile([[ </ ]])
            end)

            assert.False(ok)

            assert.match("Inline_spec.lua", tostring(err))
        end)

        it("in code that is transpiled fine but is not valid lua", function ()
            local ok, err = pcall(function ()
                return Inline:transpile([[ <>{a and }</> ]])
            end)

            assert.False(ok)
            assert.Not.match("\"inline Luax\"", tostring(err))
        end)

        -- TODO check for execution errors (needs love)
    end)
end)
