...
if data then assert(self:setData(data,source)) end end
...
if  not result[1] then return result:unpack() end
for _,g in pairs(self.gotos) do if  not self.labels[g.name] then return false,"line " .. g.span.to.line .. ": no visible label '" .. g.name .. "' for <goto>" end end
return true end
...
if self:canbe("function","keyword") then local namevar=self:parse_var()
if  not namevar then error{["msg"] = "expected name"} end
return self:node("_local",{self:makeFunction(namevar,table.unpack((assert(self:parse_funcbody(),{["msg"] = "expected function body"})))):setspan{["from"] = ffrom,["to"] = self:getloc()}}):setspan{["from"] = from,["to"] = self:getloc()} else local afrom=self:getloc()
local namelist=assert(self:parse_attnamelist(),{["msg"] = "expected attr name list"})
if self:canbe("=","symbol") then local explist=assert(self:parse_explist(),{["msg"] = "expected expr list"})
...
return self:makeFunction(funcname,table.unpack((assert(self:parse_funcbody(),{["msg"] = "expected function body"})))):setspan{["from"] = from,["to"] = self:getloc()} elseif self:canbe("for","keyword") then local namelist=assert(self:parse_namelist(),{["msg"] = "expected name list"})
if self:canbe("=","symbol") then assert.eq( # namelist,1,{["msg"] = "expected only one name in for loop"})
...
return self:node("_foreq",namelist[1],explist[1],explist[2],explist[3],table.unpack(block)):setspan{["from"] = from,["to"] = self:getloc()} elseif self:canbe("in","keyword") then local explist=assert(self:parse_explist(),{["msg"] = "expected expr list"})
self:mustbe("do","keyword")
local block=assert(self:parse_block"for in",{["msg"] = "expected block"})
self:mustbe("end","keyword")
return self:node("_forin",namelist,explist,table.unpack(block)):setspan{["from"] = from,["to"] = self:getloc()} else error{["msg"] = "'=' or 'in' expected"} end elseif self:canbe("if","keyword") then local cond=assert(self:parse_exp(),{["msg"] = "unexpected symbol"})
self:mustbe("then","keyword")
local block=self:parse_block()
local stmts=table(block)
local efrom=self:getloc()
while self:canbe("elseif","keyword") do local cond=assert(self:parse_exp(),{["msg"] = "unexpected symbol"})
self:mustbe("then","keyword")
stmts:insert(self:node("_elseif",cond,table.unpack((assert(self:parse_block(),{["msg"] = "expected block"})))):setspan{["from"] = efrom,["to"] = self:getloc()})
efrom=self:getloc() end
if self:canbe("else","keyword") then stmts:insert(self:node("_else",table.unpack((assert(self:parse_block(),{["msg"] = "expected block"})))):setspan{["from"] = efrom,["to"] = self:getloc()}) end
self:mustbe("end","keyword")
return self:node("_if",cond,table.unpack(stmts)):setspan{["from"] = from,["to"] = self:getloc()} elseif self:canbe("repeat","keyword") then local block=assert(self:parse_block"repeat",{["msg"] = "expected block"})
self:mustbe("until","keyword")
return self:node("_repeat",(assert(self:parse_exp(),{["msg"] = "unexpected symbol"})),table.unpack(block)):setspan{["from"] = from,["to"] = self:getloc()} elseif self:canbe("while","keyword") then local cond=assert(self:parse_exp(),{["msg"] = "unexpected symbol"})
self:mustbe("do","keyword")
local block=assert(self:parse_block"while",{["msg"] = "expected block"})
self:mustbe("end","keyword")
return self:node("_while",cond,table.unpack(block)):setspan{["from"] = from,["to"] = self:getloc()} elseif self:canbe("do","keyword") then local block=assert(self:parse_block(),{["msg"] = "expected block"})
self:mustbe("end","keyword")
...
while self:canbe(",","symbol") do local var=assert(self:parse_prefixexp(),{["msg"] = "expected expr"})
...
return self:node("_assign",vars,(assert(self:parse_explist(),{["msg"] = "expected expr"}))):setspan{["from"] = from,["to"] = self:getloc()} end
...
if  not ({["while"] = 1,["repeat"] = 1,["for ="] = 1,["for in"] = 1})[self.blockStack:last()] then error{["msg"] = "break not inside loop"} end
return self:node"_break":setspan{["from"] = from,["to"] = self:getloc()} end
function LuaParser:parse_funcname() local from=self:getloc()
...
return name end
function LuaParser:parse_var() local from=self:getloc()
local name=self:canbe(nil,"name")
if  not name then return  end
return self:node("_var",name):setspan{["from"] = from,["to"] = self:getloc()} end
function LuaParser:parse_namelist() local var=self:parse_var()
if  not var then return  end
local names=table{var}
while self:canbe(",","symbol") do names:insert((assert(self:parse_var(),{["msg"] = "expected name"}))) end
return names end
...
while self:canbe(",","symbol") do exps:insert((assert(self:parse_exp(),{["msg"] = "unexpected symbol"}))) end
return exps end
...
if rule.nextLevel then nextLevel=self.parseExprPrecedenceRulesAndClassNames:find(nil,function (level) return level.name == rule.nextLevel end) or error{["msg"] = "couldn't find precedence level named " .. tostring(rule.nextLevel)} end
return self:node(rule.className,(assert(self:parse_expr_precedenceTable(nextLevel),{["msg"] = "unexpected symbol"}))):setspan{["from"] = from,["to"] = self:getloc()} end
...
if rule then local nextLevel=i
if rule.nextLevel then nextLevel=self.parseExprPrecedenceRulesAndClassNames:find(nil,function (level) return level.name == rule.nextLevel end) or error{["msg"] = "couldn't find precedence level named " .. tostring(rule.nextLevel)} end
a=self:node(rule.className,a,(assert(self:parse_expr_precedenceTable(nextLevel),{["msg"] = "unexpected symbol"}))):setspan{["from"] = a.span.from,["to"] = self:getloc()} end
return a end end
...
local from=self:getloc()
if self:canbe("...","symbol") then assert.eq(self.functionStack:last(),"function-vararg",{["msg"] = "unexpected symbol"})
...
local from=self:getloc()
if self:canbe("(","symbol") then local exp=assert(self:parse_exp(),{["msg"] = "unexpected symbol"})
self:mustbe(")","symbol")
prefixexp=self:node("_par",exp):setspan{["from"] = from,["to"] = self:getloc()} else prefixexp=self:parse_var()
if  not prefixexp then return  end end
while true do if self:canbe("[","symbol") then prefixexp=self:node("_index",prefixexp,(assert(self:parse_exp(),{["msg"] = "unexpected symbol"})))
self:mustbe("]","symbol")
...
return self:makeFunction(nil,table.unpack((assert(self:parse_funcbody(),{["msg"] = "expected function body"})))):setspan{["from"] = from,["to"] = self:getloc()} end
...
if self:canbe("[","symbol") then local keyexp=assert(self:parse_exp(),{["msg"] = "unexpected symbol"})
self:mustbe("]","symbol")
self:mustbe("=","symbol")
...
if  not exp then return  end
if self.ast._var:isa(exp) and self:canbe("=","symbol") then return self:node("_assign",{self:node("_string",exp.name):setspan(exp.span)},{(assert(self:parse_exp(),{["msg"] = "unexpected symbol"}))}):setspan{["from"] = from,["to"] = self:getloc()} else return exp end end
